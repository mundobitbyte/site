(() => {
  ...
  function decorateChapter01(root) {
    ...
    if (!anchor || $('[data-review-addition="bio01-conservacao"]', root)) return;

    anchor.insertAdjacentHTML('afterend', `
      <section data-review-addition="bio01-conservacao">
        <h3>8A. Preservar biodiversidade também exige proteger território e água</h3>
        <p>Uma ação de conservação não protege apenas espécies isoladas. Ela precisa considerar <strong>hábitats, conectividade, cursos d’água, uso do solo e populações humanas</strong>. Por isso unidades de conservação, corredores ecológicos, recuperação de vegetação e planejamento de bacias hidrográficas podem funcionar como partes de uma mesma estratégia.</p>

        <div class="two-col">
          <div class="example-box"><strong class="card-title">Unidades de conservação</strong><p>São áreas protegidas com objetivos e regras de uso definidos. Categorias diferentes podem priorizar proteção mais restrita ou conciliar conservação e usos sustentáveis. Não é correto imaginar que toda área protegida tenha exatamente as mesmas regras ou exclua qualquer presença humana.</p></div>
          <div class="example-box"><strong class="card-title">Bacia hidrográfica</strong><p>É a área do terreno cuja água escoa para um mesmo sistema de drenagem. O que acontece nas partes mais altas — desmatamento, impermeabilização, erosão, lançamento de resíduos — pode repercutir quilômetros adiante.</p></div>
        </div>

        <div class="visual-box">
          <strong class="card-title">A água conecta ações feitas em lugares diferentes</strong>
          <svg class="lesson-visual" viewBox="0 0 760 300" role="img" aria-label="Esquema de uma bacia hidrográfica mostrando áreas altas, afluentes, mata ciliar, área urbana e rio principal, indicando que água e materiais transportados convergem para jusante.">
            <defs><marker id="bioReviewBasinArrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto"><path d="M0 0 L10 5 L0 10z" fill="#2f7d32"/></marker></defs>
            <path d="M45 235 L170 85 L280 190 L380 65 L505 190 L625 95 L715 235 Z" fill="#e8f3e4" stroke="#9ab79b" stroke-width="2"/>
            <path d="M170 110 C215 145 240 175 300 215" fill="none" stroke="#3b8bc2" stroke-width="7"/>
            <path d="M380 90 C390 145 385 175 350 220" fill="none" stroke="#3b8bc2" stroke-width="7"/>
            <path d="M625 120 C580 155 535 185 460 225" fill="none" stroke="#3b8bc2" stroke-width="7"/>
            <path d="M300 215 C390 235 480 230 690 250" fill="none" stroke="#2877aa" stroke-width="11"/>
            <rect x="505" y="165" width="78" height="48" rx="7" fill="#d6dbe2" stroke="#8b96a4"/><text x="544" y="192" text-anchor="middle" font-size="12" font-weight="900">cidade</text>
            <path d="M545 218 C560 228 575 235 598 241" fill="none" stroke="#b42318" stroke-width="3" marker-end="url(#bioReviewBasinArrow)"/>
            <text x="370" y="282" text-anchor="middle" font-size="13">afluentes convergem para o rio principal • impactos podem seguir para jusante</text>
          </svg>
          <p class="visual-caption">Esquema didático. Bacias reais possuem relevo, aquíferos, usos do solo e redes de drenagem muito mais complexos.</p>
        </div>

        <div class="note-box"><strong>Conservação não é apenas “proibir”</strong><p>Uma proposta responsável começa pelo diagnóstico do problema e pode combinar proteção legal, saneamento, fiscalização, restauração, manejo, educação, participação comunitária e monitoramento. A ação adequada depende do mecanismo que está causando a perda de biodiversidade.</p></div>
  ...
  function decorateChapter05(root) {
    ...
    if (!anchor || $('[data-review-addition="bio05-interferencias"]', root)) return;

    anchor.insertAdjacentHTML('afterend', `
      <section data-review-addition="bio05-interferencias">
        <h3>9A. Atividades humanas alteram ciclos por mecanismos diferentes</h3>
        <p>Dizer que “a agricultura altera os ciclos” é verdadeiro, mas ainda vago. Para avaliar impacto e mitigação, precisamos identificar <strong>qual substância está sendo movimentada, por qual rota e em qual quantidade</strong>.</p>

        <div class="three-col">
          <div class="example-box"><strong class="card-title">Agrotóxicos</strong><p>O comportamento depende da substância, formulação, dose, solo, clima e modo de aplicação. Alguns compostos podem atingir organismos não alvo ou alcançar solo e água. Não existe um único destino ambiental para todos os produtos.</p></div>
          <div class="example-box"><strong class="card-title">Pecuária</strong><p>Envolve fluxos de carbono e nitrogênio associados a alimento, dejetos, uso do solo e, em ruminantes, produção de metano pela fermentação entérica. O impacto depende do sistema produtivo e do manejo.</p></div>
          <div class="example-box"><strong class="card-title">Fertilizantes</strong><p>Nutrientes são necessários à produção vegetal, mas o excedente que não é absorvido pode ser perdido por escoamento, lixiviação ou transformações microbianas.</p></div>
        </div>

        <div class="two-col">
          <div class="example-box"><strong class="card-title">Reflorestamento</strong><p>É o estabelecimento de cobertura arbórea em uma área. O resultado ecológico depende das espécies, diversidade, finalidade, manejo e paisagem em que o plantio ocorre.</p></div>
          <div class="example-box"><strong class="card-title">Restauração ecológica</strong><p>Busca recuperar funções, processos, diversidade e relações ecológicas degradadas. Plantar árvores pode fazer parte da restauração, mas os termos não são automaticamente sinônimos.</p></div>
        </div>

        <div class="ok-box"><strong>O princípio é sempre o mesmo</strong><p>Uma medida mitigatória funciona melhor quando interrompe ou reduz a rota causal relevante: fonte → transporte → exposição/ecossistema → efeito.</p></div>

        ${q('C','Mitigação','Uma lavoura perde nutrientes para um córrego após chuvas. Qual conjunto de ações ataca mais diretamente esse mecanismo?',[['a','ajustar dose e época de adubação, manter solo coberto e proteger faixas ripárias'],['b','aumentar a aplicação antes de toda chuva'],['c','retirar toda vegetação das margens para acelerar o escoamento']],'a','Correto. O conjunto reduz excedentes e o transporte de nutrientes até a água.','A mitigação deve atuar sobre a fonte e a rota de transporte identificadas.')}
      </section>`);
  }
  ...
  function decorateChapter07(root) {
    ...
    anchor.insertAdjacentHTML('afterend', `
      <section data-review-addition="bio07-especie">
        <h3>4A. “Espécie” é um conceito científico útil — e depende do problema investigado</h3>
        <p>No <strong>conceito biológico de espécie</strong>, muito usado em organismos de reprodução sexuada, populações pertencem à mesma espécie quando há possibilidade de cruzamento e fluxo gênico em condições naturais, mantendo isolamento reprodutivo em relação a outros grupos.</p>
        <p>Esse conceito não resolve todos os casos. Organismos assexuados, fósseis e linhagens que hibridizam exigem outros critérios, como características morfológicas, ecológicas, genéticas e filogenéticas. Por isso taxonomistas trabalham com múltiplas evidências.</p>

        <div class="concept-flow">
          <div><strong>Populações conectadas</strong><span>fluxo gênico</span></div><div class="flow-arrow">→</div>
          <div><strong>Barreira aumenta</strong><span>geográfica, ecológica, comportamental...</span></div><div class="flow-arrow">→</div>
          <div><strong>Divergência</strong><span>isolamento reprodutivo pode se acumular</span></div>
        </div>
  ...
  function decorateChapter99(root) {
    if ($('[data-review-addition="bio99-lapidacao"]', root)) return;
  ...
  function applyReview(config, root) {
    ...
    if (title === 'Por que mexer em uma espécie pode alterar todo o ecossistema?') decorateChapter01(root);
    if (title === 'Por que fertilizante demais pode tirar oxigênio da água?') decorateChapter05(root);
    if (title === 'Como sabemos que populações mudam ao longo das gerações?') decorateChapter07(root);
    if (title === 'Você consegue usar a Biologia para explicar sistemas vivos?') decorateChapter99(root);
